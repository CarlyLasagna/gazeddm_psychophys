// PSYCHOPHYS STAN MODEL 6 (GAZE MODELS ONLY)
// (Model added during paper revisions o supplement)
...
// no responses are assumed to be equal and drift rates are allowed to vary
// by the gender of the stimulus.
...
data {
  ...
  array[N_obs] int gender;  // gender id of stimuli for each trial ; 1=female,2=male
...
parameters { 
  ...
  vector[N_groups] mu_grp_delta_male_pr;      // drift rate - male - group mean
  vector[N_groups] mu_grp_delta_female_pr;    // drift rate - female - group mean
  ...
  vector[N_subj] sub_delta_male_pr;    // drift rate subject mean - male stimuli
  vector[N_subj] sub_delta_female_pr;  // drift rate subject mean - female stimuli
  vector[N_subj] sub_ndt_pr;           // non-decision time subject mean in sec
  
  //parameter indexing effect of signal strength on delta
  vector[N_groups] mu_grp_b1_male_pr;      // group level hyperparamter - male stimuli
  vector[N_groups] mu_grp_b1_female_pr;    // group level hyperparamter - female stimuli
  vector<lower=0>[N_groups] sig_grp_b1_pr; // group level between subject variance
  vector[N_subj] sub_b1_male_pr;           // subject level parameter - male stimuli
  vector[N_subj] sub_b1_female_pr;         // subject level parameter - female stimuli
}
...
transformed parameters { 
  ...
  vector[N_subj] sub_delta_male;                                 // drift rate TRANSFORMED subject mean (not yet scaled!) -male stimuli
  vector[N_subj] sub_delta_female;                               // drift rate TRANSFORMED subject mean (not yet scaled!) -female stimuli
  vector<lower=rtBound,upper=max(minRT)*0.98>[N_subj] sub_ndt;   // non-decision time in sec TRANSFORMED subject mean
  vector[N_subj] sub_b1_male;                                    // effect of gaze angle on drift rate - male stimuli
  vector[N_subj] sub_b1_female;                                  // effect of gaze angle on drift rate - female stimuli
  ...
  for (i in 1:N_subj) { 
    ...
    sub_b1_male[i] = mu_grp_b1_male_pr[subj_group[i]]+sig_grp_b1_pr[subj_group[i]]*sub_b1_male_pr[i];
    sub_b1_female[i] = mu_grp_b1_female_pr[subj_group[i]]+sig_grp_b1_pr[subj_group[i]]*sub_b1_female_pr[i];
...
model {
  ...
  mu_grp_delta_male_pr ~ normal(0, 1);     // prior on drift rate group mean - male stimuli
  mu_grp_delta_female_pr ~ normal(0, 1);   // prior on drift rate group mean - female stimuli
  mu_grp_ndt_pr ~ normal(0, 1);            // prior on NDT group mean
  mu_grp_b1_male_pr ~ normal(0,1);         // prior on modulation of drift rate by signal strength (group mean) -male stimuli
  mu_grp_b1_female_pr ~ normal(0,1);       // prior on modulation of drift rate by signal strength (group mean) -female stimuli
  ...
  sub_delta_male_pr  ~ normal(0, 1);     // prior on untransformed drift rate subj mean -male stimuli
  sub_delta_female_pr  ~ normal(0, 1);   // prior on untransformed drift rate subj mean -female stimuli
  sub_ndt_pr  ~ normal(0, 1);            // prior on untransformed NDT subj mean
  sub_b1_male_pr ~ normal(0,1);          // prior on modulation of drift rate by signal strength (subj mean) - male stimuli
  sub_b1_female_pr ~ normal(0,1);        // prior on modulation of drift rate by signal strength (subj mean) - female stimuli
  ...
  for (i in 1:N_obs){ 
    ...
    if(gender[i]==1){ //if STIM is FEMALE 
      drift = Phi(sub_delta_female[subj[i]]+sub_b1_female[subj[i]]*level[i])*10-5;
    ...
    } else{ //if STIM is MALE 
      drift = Phi(sub_delta_male[subj[i]]+sub_b1_male[subj[i]]*level[i])*10-5;
      if(choice[i]==1){ //if response is YES 
        RT[i] ~ wiener(sub_alpha[subj[i]], sub_ndt[subj[i]], sub_beta[subj[i]], drift);
      } else { //if response is NO 
        RT[i] ~ wiener(sub_alpha[subj[i]], sub_ndt[subj[i]], 1-sub_beta[subj[i]], -drift);
...
generated quantities {
  ...
  vector<lower=-5,upper=5>[N_groups] mu_delta_male = -5 + 10*Phi(mu_grp_delta_male_pr);       // drift rate group mean - male stimuli
  vector<lower=-5,upper=5>[N_groups] mu_delta_female = -5 + 10*Phi(mu_grp_delta_female_pr);   // drift rate group mean - female stimuli
