...
// modeled as lower boundary responses and Male responses are modeled as
// upper boundary responses. In this version, drift rate (delta) is NOT
...
data {
  ...
  array[N_obs] int gender;  // gender id of stim for each trial;1=male;2=female
...
parameters { 
  ...
  vector[N_groups] mu_grp_delta_female_pr;    // drift rate group mean, FEMALE stimuli
  vector[N_groups] mu_grp_delta_male_pr;      // drift rate group mean, MALE stimuli
...
transformed parameters { 
  ...
  vector[N_subj] sub_delta_female;                              // drift rate -female stimuli- TRANSFORMED subject mean (not yet scaled!)
  vector[N_subj] sub_delta_male;                                // drift rate -male stimuli- TRANSFORMED subject mean (not yet scaled!)
...
model {
  ...
  for (i in 1:N_obs){ 
    ...
    if(gender[i]==1){ //if STIMULUS is  FEMALE 
...
generated quantities {
  ...
  for (i in 1:N_groups){
    mu_delta_bias[i] = mu_delta_female[i]-(-1*mu_delta_male[i]);   
  }
