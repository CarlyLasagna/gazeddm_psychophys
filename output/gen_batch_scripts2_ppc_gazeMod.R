# generate and run batch scripts for posterior predictive checks for gaze model
      ...
      for (l in type){
        for(m in task){
          ...
          script_content <- paste0("#!/bin/bash",
                                   ...
                                   "\n#SBATCH --mail-user=XXXXX", #email address
                                   ...
                                   "\ntask=",m,
                                   "\n\n#Commands to run",
                                   "\nRscript /N/slate/clasagn/gazeddm_psychophys/output/run2_ppc_gazeMod.R $run $group $modelname $task")
