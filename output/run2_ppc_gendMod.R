...
# This script runs posterior predictive checks on the winning model for the gender condition (Model 3)
...
if(file.exists(estimated_quantile_file)){ #if file exists,load it
  ...
  # get subj*cond*choice level varaibles (ie how many trials each subject has for each gaze condition, choice)
  cond_vars <- alldata %>%
    dplyr::group_by(SubjID, Gender, Resp, Task) %>%
    dplyr::summarize(trials = n(), .groups = 'drop') %>%   # Ungroup the data automatically
    tidyr::complete(SubjID, Gender, Resp, Task, fill = list(n = 0))
  ...
  for (k in 1:n_obs){
    ...
    tmp_delta_mean<-pnorm((tmp_mu_grp_delta_pr + tmp_sub_delta_pr*tmp_sig_grp_delta_pr)+tmp_level*tmp_sub_b1)*10-5
    #add in trial-level variability (if none is estimated, it will be the same value as above)
    tmp_delta<-rnorm(subsample,mean=mean(tmp_delta_mean),sd=tmp_sig_sub_delta_pr)
    ...
    qc<-foreach(l=1:length(tmp_alpha),.combine='rbind',.packages = c("RWiener","dplyr")) %dopar% {
      ...
      while (looking==1){
        ...
        #calculate DEFECTIVE cdf for the response given on this trial (deliberately making rt here tinc+tmp_ndt bc pwiener subtracts out the ndt from the rt for us)
      ...
      #append prob of yes choice as final index in vector (that way we don't have to run this thing 2x for quantiles and choices)
