...
input_models <- c('7')
runs <- c(1)
type<-c('init_fit')#par_recovery or #init_fit, ppc, par_recover, or final_fit. init fit includes loglik extract and loo calc
task<-c('jovi') #gaze or gender or jovi
groups<-c('all')
      ...
      for (l in type){
        for(m in task){
          ...
          script_content <- paste0("#!/bin/bash",
                                   ...
                                   "\n#note: mem is total memory for entire job.mem-per-cpu is memory for each core",
                                   "\n\n#Set up environment",
                                   "\npwd; hostname; date",
                                   "\necho \"Running init fit m", i," group ",k," ",l," for m", i, '_run',j, "\"",
                                   "\n\nmodule load r/4.4",
                                   ...
                                   "\ntask=",m,
                                   "\n\n#Commands to run",
                                   "\nRscript [PATH TO RUN FILE]/run1_init_fit_jovi_censor.R $run $group $modelname $task")
