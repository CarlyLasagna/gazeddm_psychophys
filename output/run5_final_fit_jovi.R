...
iter <- 5000          # number of samples per chain (total postwarmup draws = (iter-warmup)*chains)
...
if(task=='gaze'){
  ...
  alldata$SubjID<-match(alldata$Subj, unique(alldata$Subj)) #subj IDs to sequential indexes for fitting
  alldata$Resp<-ifelse(alldata$Resp=="2",1,2) #recode L response (2) as 1 and R response (3) as 2
  alldata$Group<-match(alldata$Group, unique(alldata$Group))
  alldata <- alldata[order(alldata$SubjID, alldata$Resp), ]
}
...
data_stan<-list(
  ...
  choice=alldata$Resp, # choice for each observation; 1=Left,2=Right
...
sample_file<-paste0(finaldir,'/allgroups_final_fit_samples.csv')
