...
rename_corr_pars<-function(pars){
  ...
  pars <- gsub("delta_gend", "Drift Rate (Gender)", pars)
...
plot_parrecover_subj<-function(data,par,parname,sim_id,genMean_suffix,simMean_suffix, simHDI_suffix,recover_suffix){
  ...
  cor_coef<-cor(data[[paste0(par,simMean_suffix)]],data[[paste0(par,genMean_suffix)]])
...
plot_posterior_diffs<-function(data,ci,par_prefix,rawpar_prefix,sig_prefix,par_name,group_nums,group_names,sample_sizes,round_digits,showtext){
  ...
  scale_color_manual(values=c('#318799','#6a4894'))+
  ylab(par_name)+
...
if(file.exists(outfile)==FALSE){
  ...
  #remove erroneously fast responses or slow responses 
...
if(file.exists(outfile)==FALSE){
  ...
  # extract social cognition principal component from er40/rme/msceit data with missing values (winsorize 2x extreme er40 values)
  ...
  # tmp_data<-data.frame(msceit=as.numeric(sg2_correlates$msceit),
  #                      er40=Winsorize(as.numeric(sg2_correlates$er40_correctresp), 
  #                                     probs = c(0.02, 1),na.rm=TRUE),
  #                      rme=Winsorize(as.numeric(sg2_correlates$rme_accuracy), 
  #                                    probs = c(0.02, 1),na.rm=TRUE))
  ...
  ## calculate functioning principal component
  tmp_data<-data.frame(sspa=as.numeric(sg2_correlates$sspa),
                       er40=as.numeric(sg2_correlates$gf_social))
  
  sg2_correlates$func_pc<-pcamethods_bpca(data=tmp_data,npcs=1,scale="uv",center=TRUE,method="pcamethods")$PC1
...
For the self-referential gaze perception condition, the winning model was one in which evidence accumulation was modulated by the eye contact signal strength of the stimuli. It captured two distinct aspects of the evidence accumulation process. First, the drift sensitivity<sup>gaze</sup> indexed how evidence accumulation varied in response to physical changes in self-referential gaze signals. Those with higher values are more data-driven and attuned to environmental signals, as they better translate stronger social signals into more efficient self-referential perception. Second, the drift bias<sup>gaze</sup> captured perceptual bias for direct/indirect gaze regardless of physical changes in the signal, where a drift bias<sup>gaze</sup> > 0 indicates a perceptual bias for direct gaze, and a drift bias<sup>gaze</sup> < 0 indicates a perceptual bias for indirect gaze. Other cognitive processes of the DDM—-threshold separation<sup>gaze</sup> (response caution), start point<sup>gaze</sup> (expectancy bias), and NDT<sup>gaze</sup> (motor/sensory processes)—-were assumed to be trait-level processes that did not vary in response to changes in the gaze signal. 
...
For the non-self-referential gender perception condition, the winning model (“Model 3: Gender”) was one in which evidence accumulation (drift rate) was influenced by the gender of stimuli, but not other task-irrelevant social cues (i.e., gaze). The effect of gender on evidence accumulation was captured via separate drift rates for male and female stimuli (drift rate<sup>gend-M</sup> and drift rate<sup>gend-F</sup>). As “male” responses were coded as the top boundary and “female” responses were coded as the bottom boundary, higher values for drift rate<sup>gend-M</sup> and lower values for drift rate<sup>gend-F</sup> reflected more efficient evidence accumulation. This model also captured perceptual biases (drift bias<sup>gend</sup>) for male/female stimuli, regardless of physical changes in the gender of the stimulus, where drift bias<sup>gend</sup> > 0 indicates a perceptual bias for males, while drift bias<sup>gend</sup> < 0 indicates a perceptual bias for females. Finally, other cognitive processes of the DDM—threshold separation (response caution), start point (expectancy bias), and NDT (motor/sensory processes)—were assumed to be trait-level processes that did not vary in response to changes in the gender of stimuli. 

The parameters of this model are largely comparable to the winning gaze model, with one exception: the self-referential gaze perception model estimates drift sensitivity<sup>gaze</sup>, while the non-self-referential gender perception model estimates. Drift sensitivity can only be estimated when target stimulus features, like eye gaze, are manipulated continuously, whereas separate drift rates can only be estimated when stimulus features, like gender, are manipulated categorically. Both classes of parameters are highly correlated (r > |0.6|; see intercorrelations below) and capture similar processes. The primary distinction is that drift rate<sup>gend-M</sup> and drift rate<sup>gend-F</sup> capture overall rates of evidence accumulation and sensitivity to changes in the gender signal concurrently, while drift sensitivity<sup>gaze</sup> represents a purer measure of sensitivity to changes in the gaze signal.
...
tmp_pars<-c('delta_bias','delta_bias_gend',
            'delta_b1','delta_gend','delta_female_gend','delta_male_gend',
            'beta','beta_gend',
            'alpha','alpha_gend')
...
Results showed that both models showed good recovery of parameters across SZ and HC at the group- (88-100% recovery) and subject-level (93-95% recovery). 
...
#Cohen’s 1988 guidelines of effect sizes:
#              small   medium   large
#Cohen’s   d = 0.20    0.50     0.80 
#Increment.f²  0.02	   0.15	    0.35	
...
for(i in 1:n_subj){
  ...
  gaze3_all_samples[[temp_col3]]<-gaze3_all_samples[[temp_col1]]-(gaze3_all_samples[[temp_col2]]*-1)
  
  temp_col4<-paste0("sub_delta.",i,".")
  gaze3_all_samples[[temp_col4]]<-rowMeans(cbind(gaze3_all_samples[[temp_col1]],(gaze3_all_samples[[temp_col2]]*-1)))
}
...
gaze3_all_samples$mu_grp_delta_bias_pr.1.<-gaze3_all_samples$mu_grp_delta_male_pr.1.*-1-gaze3_all_samples$mu_grp_delta_female_pr.1.
gaze3_all_samples$mu_grp_delta_bias_pr.2.<-gaze3_all_samples$mu_grp_delta_male_pr.2.*-1-gaze3_all_samples$mu_grp_delta_female_pr.2.
...
params<-c('mu_alpha.','mu_beta.','mu_ndt.','mu_delta_female.','mu_delta_male.','mu_delta_bias.')
raw_params<-c('mu_grp_alpha_pr.','mu_grp_beta_pr.','mu_grp_ndt_pr.','mu_grp_delta_female_pr.','mu_grp_delta_male_pr.','mu_grp_delta_bias_pr.')
sig_params<-c('sig_grp_alpha_pr.','sig_grp_beta_pr.','sig_grp_ndt_pr.','sig_grp_delta_pr.','sig_grp_delta_pr.','sig_grp_delta_pr.')
param_names<-c('Threshold Separation','Start Point','NDT','Drift: Female Faces','Drift: Male Faces','Drift Bias')
...
```{r id99332122322999, fig.height=3.75, fig.width=2.5, cache=TRUE, dpi=500,include=FALSE}
...
params<-c('mu_alpha.','mu_beta.','mu_ndt.','mu_delta_bias.')
raw_params<-c('mu_grp_alpha_pr.','mu_grp_beta_pr.','mu_grp_ndt_pr.','mu_grp_delta_bias_pr.')
sig_params<-c('sig_grp_alpha_pr.','sig_grp_beta_pr.','sig_grp_ndt_pr.','sig_grp_delta_pr.')
param_names<-c('Threshold Separation','Start Point','NDT','Drift Bias')
...
ylims<-list(c(-3.5,3.5),c(-3.5,3.5))
params<-c('mu_delta_female.','mu_delta_male.')
raw_params<-c('mu_grp_delta_female_pr.','mu_grp_delta_male_pr.')
sig_params<-c('sig_grp_delta_pr.','sig_grp_delta_pr.')
param_names<-c('Drift Rate: Male & Female Faces','Drift Rate: Male & Female Faces')

  p1<-plot_posterior_diffs(
    data=gaze3_all_samples, 
    ci=.95, #credible interval
    par_prefix=params[1],
    rawpar_prefix=raw_params[1],
    sig_prefix=sig_params[1],
    par_name = param_names[1],
    group_nums = c(1,2),
    group_names=c('HC','SZ'),
    sample_sizes=c(39,42),
    round_digits=3,
  ...
  p2<-plot_posterior_diffs(
    data=gaze3_all_samples, 
    ci=.95, #credible interval
    par_prefix=params[2],
    rawpar_prefix=raw_params[2],
    sig_prefix=sig_params[2],
    par_name = param_names[2],
    group_nums = c(1,2),
    group_names=c('HC','SZ'),
    sample_sizes=c(39,42),
    round_digits=3, 
...
p_combined <- p1 + p2$layers
print(p_combined + coord_cartesian(ylim = ylims[[1]]))
...
effect_names<-c("Threshold Separation (Gaze)", "Start Point (Gaze)", "NDT (Gaze)","Drift Sensitivity (Gaze)", "Drift Bias (Gaze)",
                'Threshold Separation (Gender)','Start Point (Gender)','NDT (Gender)','Drift: Female Faces (Gender)','Drift: Male Faces','Drift Bias (Gender)')
effect_sizes<-c(-1.07,.472,.011, .881,-1.062,.349,-.876,-.961,.974,-.69,-.284)
...
effect_names<-c("GPTS: Referential Thinking - Drift Sensitivity (Gaze)", "GPTS: Referential Thinking - Drift Bias (Gaze)", "GPTS: Referential Thinking - Start Point (Gaze)","GPTS: Referential Thinking - Drift Rate (Gender)",
                "GPTS: Persecution - Drift Sensitivity (Gaze)", "GPTS: Persecution - Drift Bias (Gaze)", "GPTS: Persecution - Start Point (Gaze)","GPTS: Persecution - Drift Rate (Gender)",
                "Social Cognition - Drift Sensitivity (Gaze)", "Social Cognition - Drift Bias (Gaze)", "Social Cognition - Start Point (Gaze)","Social Cognitionn - Drift Rate (Gender)")
effect_sizes<-c(.13,.03,.01,.21,.07,.03,.02,.19,.03,.02,.02,.01)
