...
subtitle: "Modeling Self-Reference in Schizophrenia: General and Specific Mechanisms
  Underlying Decision-Making are Differentially Associated with Paranoia"
output:
  html_document:
    toc: yes
    toc_float: yes
...
Below are sample stimuli for the gender identification task. During modeling, gender was effects coded such that "female" responses were coded as 2 (lower boundary responses) and "male" responses were coded as 1 (upper boundary responses).  
...
*__Note:__ Throughout this section, "Model 2 (Eye Contact)" refers to the  model for the eye contact detection task described in the main text, "Model 3 (Gender)" refers to the model for the gender identification task described in the main text, and "Model 5 (Visual Integration)" refers to the model for the visual integration task described in the main text.*
...
Initial fits of all models were run on a high performance computing cluster using 1500 warmup samples, 1000 postwarmup draws per chain, 10 chains, resulting in 10000 postwarmup draws in cmdstanr. These "initial fits" were used as the basis for all model evaluation steps described below. For analyses, each winning model was run using a larger number of samples (1500 warmup samples, 5000 postwarmup draws per chain, 10 chains, resulting in 50000 postwarmup draws). This ensured that our inferences were based on stable posteriors for all parameters.

# Model Specification

Model 2 (Eye Contact) is the model for the eye contact detection task described in the main text, Model 3 (Gender) is the model for the gender identification task described in the main text, and Model 5 (Visual Integration) is the model Visual Integration task described in the main text. Note that Model 2 (Eye Contact) and Model 5 (Visual Integration) share the same structure. Stimulus-coding is used to characterize participants' responses on all tasks. All models used weakly information priors which included N(0,1) for all group- and subject-level mean parameters and positively bound N(0,0.2) for all variance parameters.
...
As such, the model evaluation steps are performed on the gender models with separate male/female drift rate parameters. For  analyses  we recast those parameters in the gender identification task to drift sensitivity (discriminability) and drift bias (as shown above).
...
models<-c('m1_psychophys','m2_psychophys','m6_psychophys')
groupnames<-c('HC','SZ')
...
This 'effects coding approach' is equivalent to the following calculations of drift sensitivity and drift bias for the gender identification task, which is how we calculated these parameters:
...
We assessed convergence for all models by ensuring trace plots showed well-mixed chains, Rhat values were < 1.01 for all parameters, autocorrelation was ~0 by a lag of 30, and there were no divergences. This was true of all parameters for all models tested. For brevity, we only show the convergence check outputs below for the winning models.
...
Rhat values for all parameters were <= 1.01.

```{r id342809374873412, cache=TRUE, include=FALSE}
##
...
gaze_samples<-gaze2_all_samples[,-grep("beta_pr|delta_pr|alpha_pr|ndt_pr|absent_pr|present_pr|draw|iteration|lp|chain|sig_",colnames(gaze2_all_samples))]
...
```{r id342, cache=TRUE, include=FALSE}
...
gender_samples<-gender3_all_samples[,-grep("beta_pr|delta_pr|alpha_pr|ndt_pr|male_pr|female_pr|draw|iteration|lp|chain|sig_",colnames(gender3_all_samples))]
params<-colnames(gender_samples)

#flip orientation because male was originally coded as bottom boundary for intial fits.
gender_samples$mu_delta_male.1.<-gender_samples$mu_delta_male.1.*-1
gender_samples$mu_delta_male.2.<-gender_samples$mu_delta_male.2.*-1
gender_samples$mu_delta_female.1.<-gender_samples$mu_delta_female.1.*-1
gender_samples$mu_delta_female.2.<-gender_samples$mu_delta_female.2.*-1
gender_samples$mu_beta.1.<-(1-gender_samples$mu_beta.1.)
gender_samples$mu_beta.2.<-(1-gender_samples$mu_beta.2.)
...
```{r id32242, cache=TRUE, include=FALSE}
...
jovi_samples<-jovi_all_samples[,-grep("beta_pr|delta_pr|alpha_pr|ndt_pr|_L_pr|_R_pr|draw|iteration|lp|chain|sig_",colnames(jovi_all_samples))]
...
### Group Parameters (Averaged over simulations)
...
p <- ggplot(data = plot_data2, aes(x = group, y = sim_mean, color = group)) +
  ...
  ggtitle("Eye Contact: Threshold Sep\n(Averaged over 50 sims)") + 
...
### Group Parameters (Separate for each simulation)
...
params<-c('alpha','beta','delta','b1')
parnames<-c('Eye Contact: Threshold Sep','Eye Contact: Start Point','Eye Contact: Drift Bias','Eye Contact: Drift Sensitivity')
groups<-c('HC','SZ')
...
#BETA (flip orientation because male was originally coded as bottom boundary for initial fits)
plot_data3<-data.frame(group=c('HC','SZ'),
                       sim_mean=1-c(plot_data$beta_grp_sim_mean1,plot_data$beta_grp_sim_mean2),
                       gen_mean=1-c(plot_data$beta_grp_gen1,plot_data$beta_grp_gen2),
                       sim_hdi_lo=1-c(plot_data$beta_grp_sim_hdi_lo1,plot_data$beta_grp_sim_hdi_lo2),
                       sim_hdi_hi=1-c(plot_data$beta_grp_sim_hdi_hi1,plot_data$beta_grp_sim_hdi_hi2))
...
#DELTA female (flip orientation because male was originally coded as bottom boundary for initial fits)
plot_data3<-data.frame(group=c('HC','SZ'),
                       sim_mean=-1*c(plot_data$delta_female_grp_sim_mean1,plot_data$delta_female_grp_sim_mean2),
                       gen_mean=-1*c(plot_data$delta_female_grp_gen1,plot_data$delta_female_grp_gen2),
                       sim_hdi_lo=-1*c(plot_data$delta_female_grp_sim_hdi_lo1,plot_data$delta_female_grp_sim_hdi_lo2),
                       sim_hdi_hi=-1*c(plot_data$delta_female_grp_sim_hdi_hi1,plot_data$delta_female_grp_sim_hdi_hi2))
...
#DELTA male (flip orientation because male was originally coded as bottom boundary for initial fits)
plot_data3<-data.frame(group=c('HC','SZ'),
                       sim_mean=-1*c(plot_data$delta_male_grp_sim_mean1,plot_data$delta_male_grp_sim_mean2),
                       gen_mean=-1*c(plot_data$delta_male_grp_gen1,plot_data$delta_male_grp_gen2),
                       sim_hdi_lo=-1*c(plot_data$delta_male_grp_sim_hdi_lo1,plot_data$delta_male_grp_sim_hdi_lo2),
                       sim_hdi_hi=-1*c(plot_data$delta_male_grp_sim_hdi_hi1,plot_data$delta_male_grp_sim_hdi_hi2))
...
#flip orientation as male was originally coded as the lower boundary for intial fits
group_summary$beta_grp_gen1<-(1-group_summary$beta_grp_gen1)
group_summary$beta_grp_gen2<-(1-group_summary$beta_grp_gen2)
group_summary$beta_grp_sim_hdi_hi1<-(1-group_summary$beta_grp_sim_hdi_hi1)
group_summary$beta_grp_sim_hdi_hi2<-(1-group_summary$beta_grp_sim_hdi_hi2)
group_summary$beta_grp_sim_hdi_lo1<-(1-group_summary$beta_grp_sim_hdi_lo1)
group_summary$beta_grp_sim_hdi_lo2<-(1-group_summary$beta_grp_sim_hdi_lo2)
group_summary$beta_grp_sim_mean1<-(1-group_summary$beta_grp_sim_mean1)
group_summary$beta_grp_sim_mean2<-(1-group_summary$beta_grp_sim_mean2)

group_summary$delta_female_grp_gen1<-(-1*group_summary$delta_female_grp_gen1)
group_summary$delta_female_grp_gen2<-(-1*group_summary$delta_female_grp_gen2)
group_summary$delta_female_grp_sim_hdi_hi1<-(-1*group_summary$delta_female_grp_sim_hdi_hi1)
group_summary$delta_female_grp_sim_hdi_hi2<-(-1*group_summary$delta_female_grp_sim_hdi_hi2)
group_summary$delta_female_grp_sim_hdi_lo1<-(-1*group_summary$delta_female_grp_sim_hdi_lo1)
group_summary$delta_female_grp_sim_hdi_lo2<-(-1*group_summary$delta_female_grp_sim_hdi_lo2)
group_summary$delta_female_grp_sim_mean1<-(-1*group_summary$delta_female_grp_sim_mean1)
group_summary$delta_female_grp_sim_mean2<-(-1*group_summary$delta_female_grp_sim_mean2)

group_summary$delta_male_grp_gen1<-(-1*group_summary$delta_male_grp_gen1)
group_summary$delta_male_grp_gen2<-(-1*group_summary$delta_male_grp_gen2)
group_summary$delta_male_grp_sim_hdi_hi1<-(-1*group_summary$delta_male_grp_sim_hdi_hi1)
group_summary$delta_male_grp_sim_hdi_hi2<-(-1*group_summary$delta_male_grp_sim_hdi_hi2)
group_summary$delta_male_grp_sim_hdi_lo1<-(-1*group_summary$delta_male_grp_sim_hdi_lo1)
group_summary$delta_male_grp_sim_hdi_lo2<-(-1*group_summary$delta_male_grp_sim_hdi_lo2)
group_summary$delta_male_grp_sim_mean1<-(-1*group_summary$delta_male_grp_sim_mean1)
group_summary$delta_male_grp_sim_mean2<-(-1*group_summary$delta_male_grp_sim_mean2)
...
#flip orientation as male was originally coded as the lower boundary for intial fits
subj_summary$beta_sub_gen<-(1-subj_summary$beta_sub_gen)
tmp_hi<-(1-subj_summary$beta_sub_sim_hdi_hi)
tmp_lo<-(1-subj_summary$beta_sub_sim_hdi_lo)
subj_summary$beta_sub_sim_hdi_hi<-tmp_lo
subj_summary$beta_sub_sim_hdi_lo<-tmp_hi
subj_summary$beta_sub_sim_mean<-(1-subj_summary$beta_sub_sim_mean)

subj_summary$delta_male_sub_gen<-(-1*subj_summary$delta_male_sub_gen)
tmp_hi<-(-1*subj_summary$delta_male_sub_sim_hdi_hi)
tmp_lo<-(-1*subj_summary$delta_male_sub_sim_hdi_lo)
subj_summary$delta_male_sub_sim_hdi_hi<-tmp_lo
subj_summary$delta_male_sub_sim_hdi_lo<-tmp_hi
subj_summary$delta_male_sub_sim_mean<-(-1*subj_summary$delta_male_sub_sim_mean)

subj_summary$delta_female_sub_gen<-(-1*subj_summary$delta_female_sub_gen)
tmp_hi<-(-1*subj_summary$delta_female_sub_sim_hdi_hi)
tmp_lo<-(-1*subj_summary$delta_female_sub_sim_hdi_lo)
subj_summary$delta_female_sub_sim_hdi_hi<-tmp_lo
subj_summary$delta_female_sub_sim_hdi_lo<-tmp_hi
subj_summary$delta_female_sub_sim_mean<-(-1*subj_summary$delta_female_sub_sim_mean)
...
p <- ggplot(data = plot_data2, aes(x = group, y = sim_mean, color = group)) +
  ...
  geom_errorbar(data = plot_data2, aes(ymin = sim_hdi_lo, ymax = sim_hdi_hi), 
                linewidth = .5, width = 0, alpha = 1, position = position_dodge(width = .5)) +
  coord_cartesian(ylim = c(1.55, 1.8)) +
  ggtitle("Visual Integration: Threshold Sep\n(Averaged over 50 sims)") + 
  ylab("Parameter Estimate") +
  theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust = 1)) +
  geom_point(data = data.frame(category = plot_data2$group, value = plot_data2$gen_mean), 
             aes(x = category, y = value), shape = 4, size = 4, stroke = 2, color = "red",alpha=.4) +
...
#
params<-c('alpha','beta','delta','b1')
parnames<-c('Visual Integration:\nThreshold Sep','Visual Integration:\nStart Point','Visual Integration:\nDrift Bias','Visual Integration:\nDrift Sensitivity')
groups<-c('HC','SZ')
...
#Visual Integration MODEL (with censoring)
jovi_all_samples_censor<-as.data.frame(fread('/Users/carlylasagna/University of Michigan Dropbox/Carly Lasagna/ddm_gaze_psychophys/ddm/output/hddm_m7_psychophys/jovi/init_fit/allgroups_init_fit_samples.csv'))

colnames(jovi_all_samples_censor) <- gsub("\\[|\\]", ".", colnames(jovi_all_samples_censor))

for(i in 1:n_subj){
  temp_col1<-paste0("sub_delta.",i)
  jovi_all_samples_censor[[temp_col1]]<-pnorm(jovi_all_samples_censor[[temp_col1]])*10-5
}

jovi_sub_alpha<-colMeans(jovi_all_samples_censor[,grep("sub_alpha.",fixed=T,colnames(jovi_all_samples_censor))])
jovi_sub_beta<-colMeans(jovi_all_samples_censor[,grep("sub_beta.",fixed=T,colnames(jovi_all_samples_censor))])
jovi_sub_ndt<-colMeans(jovi_all_samples_censor[,grep("sub_ndt.",fixed=T,colnames(jovi_all_samples_censor))])

jovi_sub_delta_bias<-colMeans(jovi_all_samples_censor[,grep("sub_delta.",fixed=T,colnames(jovi_all_samples_censor))])
jovi_sub_delta_b1<-colMeans(jovi_all_samples_censor[,grep("sub_b1.",fixed=T,colnames(jovi_all_samples_censor))])

sub_samples_jovi_censor<-data.frame(alpha_jovi_c=jovi_sub_alpha,
                             beta_jovi_c=jovi_sub_beta,
                             delta_bias_jovi_c=jovi_sub_delta_bias,
                             delta_b1_jovi_c=jovi_sub_delta_b1)

all_sub_data<-cbind(all_sub_data,sub_samples_jovi,sub_samples_jovi_censor)
...
outfile<-paste(datadir,"/schizgaze12_gaze_beh.csv",sep="")
gaze_data<-read.csv(outfile)
gaze_data<-subset(gaze_data,gaze_data$Study=="schizgaze2")
gaze_data<-subset(gaze_data,gaze_data$Task=="Eyes")

gaze <- gaze_data %>% 
  dplyr::group_by(Subj) %>% 
  dplyr::summarize(gaze_rt = mean(RT,na.rm=TRUE))

gaze$subj<-gaze$Subj
gaze$Subj<-NULL
all_sub_data<-merge(all_sub_data, gaze,all.x=TRUE,by.x = "subj",by.y = "subj", no.dups = T)

outfile<-paste(datadir,"/schizgaze12_gaze_beh.csv",sep="")
gend_data<-read.csv(outfile)
gend_data<-subset(gend_data,gend_data$Study=="schizgaze2")
gend_data<-subset(gend_data,gend_data$Task=="GenderID")

gend <- gend_data %>% 
  dplyr::group_by(Subj) %>% 
  dplyr::summarize(gend_rt = mean(RT,na.rm=TRUE),
                   gend_acc = mean(Acc,na.rm=TRUE))

gend$subj<-gend$Subj
gend$Subj<-NULL
all_sub_data<-merge(all_sub_data, gend,all.x=TRUE,by.x = "subj",by.y = "subj", no.dups = T)

# outfile<-paste(datadir,"/schizgaze2_jovi_beh.csv",sep="")
# jovi_data<-read.csv(outfile)
# jovi_data<-subset(jovi_data,jovi_data$Study=="schizgaze2")
# 
# jovi <- jovi_data %>% 
#   dplyr::group_by(Subj) %>% 
#   dplyr::summarize(jovi_rt = mean(RT,na.rm=TRUE),
#                    jovi_acc = mean(Acc,na.rm=TRUE))
# 
# jovi<-as.data.frame(jovi)
# 
# jovi$subj<-jovi$Subj
# jovi$Subj<-NULL
# 
# all_sub_data<-merge(all_sub_data, jovi,all.x=TRUE,by.x = "subj",by.y = "subj", no.dups = T)
...
demo_table<-dplyr::select(all_sub_data,c('subj','group','age','female_dummy','edu','parent_edu',
                                         ...
                                         'gazebayesnew_thres50','gazebayesnew_slope','gaze_rt',
                                         'gend_acc','gend_rt',
                                         'jovi_acc','jovi_rt'))
...
colnames(demo_table)<-c('Subj','Group','Age','Sex (% Female)','Education (Years)','Parent Education (Years)',
                        ...
                        'Gaze: Bias','Gaze: Sensitivity','Gaze: RT',
                        'Gender: Accuracy','Gender: RT',
                        'Visual Integration: Accuracy','Visual Integration: RT')
...
demo_table_all %>%
  ...
  pack_rows("Task Performance", 25, 31) %>%
  pack_rows("Eye Contact Detection", 25, 27,hline_before = T,italic=T,bold=F) %>%
  pack_rows("Gender Identification", 28, 29,hline_before = T,italic=T,bold=F) %>%
  pack_rows("Visual Integration", 30, 31,hline_before = T,italic=T,bold=F) %>%
  kable_styling(full_width=T)

```
 
## RT Distributions by Group
 
```{r id12433322332222222, echo=FALSE, fig.height=4,fig.width=4, message=FALSE, warning=FALSE, cache=TRUE}

library(ggplot2)
library(ggbeeswarm)  # for jittered points

outfile<-paste(datadir,"/schizgaze12_gaze_beh.csv",sep="")
plot_data<-read.csv(outfile)
plot_data<-subset(plot_data,plot_data$Study=="schizgaze2")

plot_data1<-subset(plot_data,plot_data$Task=="Eyes")

ggplot(plot_data1, aes(x = Group, y = RT, fill = Group)) +
  geom_point(position = position_jitter(width = 0.05, height = 0),alpha = 0.3, size = .5) +
  geom_violin(alpha = 0.6, trim = FALSE, color = NA) +
  labs(x = "Group", y = "RT (ms)") +
  guides(fill = "none", color = "none")+theme_bw()+ggtitle("Gaze Perception Task:\nRT Distribution")

plot_data1<-subset(plot_data,plot_data$Task=="GenderID")

ggplot(plot_data1, aes(x = Group, y = RT, fill = Group)) +
  geom_point(position = position_jitter(width = 0.05, height = 0),alpha = 0.3, size = .5) +
  geom_violin(alpha = 0.6, trim = FALSE, color = NA) +
  labs(x = "Group", y = "RT (ms)") +
  guides(fill = "none", color = "none")+theme_bw()+ggtitle("Gender Identification Task:\nRT Distribution")

outfile<-paste(datadir,"/schizgaze2_jovi_beh.csv",sep="")
plot_data<-read.csv(outfile)

ggplot(plot_data, aes(x = Group, y = RT, fill = Group)) +
  geom_point(position = position_jitter(width = 0.05, height = 0),alpha = 0.3, size = .5) +
  geom_violin(alpha = 0.6, trim = FALSE, color = NA) +
  labs(x = "Group", y = "RT (ms)") +
  guides(fill = "none", color = "none")+theme_bw()+ggtitle("Visual Integration Task:\nRT Distribution")
...
```{r id99332122322999, fig.height=2.5, fig.width=1.5, cache=TRUE, dpi=500, include=FALSE}
...
## Bayesian Mixed ANOVA (Task Specificity of Group Effects)
 
### Drift Sensitivity
 
```{r id124333223322222322, fig.height=4, fig.width=4, message=FALSE, warning=FALSE, cache=TRUE, include=FALSE}

gaze<-all_sub_data[,c("subj","sz_dummy","delta_b1","delta_bias","beta","alpha")]
gaze$task<-"gaze"
colnames(gaze)<-c("subj","group","delta","delta_bias","beta","alpha","task")
gaze$delta<-scale(gaze$delta)

gend<-all_sub_data[,c("subj","sz_dummy","delta_gend","delta_bias_gend","beta_gend","alpha_gend")]
gend$task<-"gend"
colnames(gend)<-c("subj","group","delta","delta_bias","beta","alpha","task")
gend$delta<-scale(gend$delta)

jovi<-all_sub_data[,c("subj","sz_dummy","delta_b1_jovi","delta_bias_jovi","beta_jovi","alpha_jovi")]
jovi$task<-"jovi"
colnames(jovi)<-c("subj","group","delta","delta_bias","beta","alpha","task")
jovi$delta<-scale(jovi$delta)

alltask<-rbind(gaze,gend,jovi)
alltask$group<-as.factor(ifelse(alltask$group==1,"SZ","HC"))
alltask$task<-factor(alltask$task,levels=c("gaze","gend","jovi"))

library(brms)

alltask$group<-factor(alltask$group)

out <- brm(
  delta ~ task * group +  (1 | subj),
  data   = alltask,
  family = gaussian(),   # same as lm() default
  cores  = 4,silent=2
)
...
out<-data.frame(Parameter=rownames(out),
                Estimate=round(as.numeric(out$Estimate),2),
                HDI_Low=round(out$`l-95% CI`,2),
                HDI_High=round(out$`u-95% CI`,2))

out$Credible<-""

for(i in 1:nrow(out)){
  if(out$HDI_Low[i]<0&out$HDI_High[i]<0){
    out$Credible[i]<-"*"
  }
  if(out$HDI_Low[i]>0&out$HDI_High[i]>0){
    out$Credible[i]<-"*"
...
out$Parameter<-gsub("task","Task",out$Parameter)
out$Parameter<-gsub("group","Group",out$Parameter)
out$Parameter<-gsub("gend","Gender",out$Parameter)
out$Parameter<-gsub("jovi","VisualIntegration",out$Parameter)
rownames(out)<-NULL


```

```{r id1243233223323332222322, echo=FALSE, fig.height=4, fig.width=4, message=FALSE, warning=FALSE, cache=TRUE}
###
out %>%
  kbl(caption = "Mixed ANOVA on Drift Sensitivity\n(Reference Group=HC and Reference Task=Eye Contact Detection)",valign = "t") %>%
  kable_classic(full_width=F, html_font = "Cambria") %>%
  kable_styling(full_width=T) 
...
```{r id1243233223322222322, fig.height=4, fig.width=4, message=FALSE, warning=FALSE, cache=TRUE, include=FALSE}

out <- brm(
  delta_bias ~ task * group+  (1 | subj),
  data   = alltask,
  family = gaussian(),   # same as lm() default
  cores  = 4,silent=2
)
...
```{r id12432313223322222322, echo=FALSE, fig.height=4, fig.width=4, message=FALSE, warning=FALSE, cache=TRUE}
###
out %>%
  kbl(caption = "\nMixed ANOVA on Drift Bias\n(Reference Group=HC and Reference Task=Eye Contact Detection)",valign = "t") %>%
  kable_classic(full_width=F, html_font = "Cambria") %>%
  kable_styling(full_width=T) 
...
```{r id124323323233222322322, fig.height=4, fig.width=4, message=FALSE, warning=FALSE, cache=TRUE, include=FALSE}

out <- brm(
  beta ~ task * group+  (1 | subj),
  data   = alltask,
  family = gaussian(),   # same as lm() default
  cores  = 4,silent=2
)
...
```{r id124323132233222322322, echo=FALSE, fig.height=4, fig.width=4, message=FALSE, warning=FALSE, cache=TRUE}
###
out %>%
  kbl(caption = "\nMixed ANOVA on Start Point\n(Reference Group=HC and Reference Task=Eye Contact Detection)",valign = "t") %>%
  kable_classic(full_width=F, html_font = "Cambria") %>%
  kable_styling(full_width=T) 

```

## Sensitivity - Group Differences Accounting for All Gaze/Gender Effects

### Model 6 (Eye Contact) + Gender Effects

```{r id9933212222322999, echo=FALSE, cache=TRUE,fig.width=4,fig.height=3.75}

gaze6_all_samples<-as.data.frame(fread('/Users/carlylasagna/University of Michigan Dropbox/Carly Lasagna/ddm_gaze_psychophys/ddm/output/hddm_m6_psychophys/gaze/init_fit/allgroups_init_fit_samples.csv'))

colnames(gaze6_all_samples) <- gsub("\\[|\\]", ".", colnames(gaze6_all_samples))
gaze6_all_samples<-gaze6_all_samples[,-grep("sub_",colnames(gaze6_all_samples))]

# calculate and plot group differences
params<-c('mu_alpha.','mu_beta.','mu_ndt.',
          'mu_grp_b1_male_pr.','mu_grp_b1_female_pr.',
          'mu_delta_male.','mu_delta_female.')

raw_params<-c('mu_grp_alpha_pr.','mu_grp_beta_pr.','mu_grp_ndt_pr.',
              'mu_grp_b1_male_pr.','mu_grp_b1_female_pr.',
              'mu_grp_delta_male_pr.','mu_grp_delta_female_pr.')

sig_params<-c('sig_grp_alpha_pr.','sig_grp_beta_pr.','sig_grp_ndt_pr.',
              'sig_grp_b1_pr.','sig_grp_b1_pr.',
              'sig_grp_delta_pr.','sig_grp_delta_pr.')

param_names<-c('Threshold Separation','Start Point','NDT',
               'Drift Sensitivity (Male)','Drift Sensitivity (Female)',
               'Drift Bias (Male)','Drift Bias (Female)')

#loop through and plot/compare group differences
for(i in 1:length(params)){

  p<-plot_posterior_diffs(
    data=gaze6_all_samples,
    ci=.95, #credible interval
    par_prefix=params[i],
    rawpar_prefix=raw_params[i],
    sig_prefix=sig_params[i],
...
### Model 4 (Gender) + Gaze Effects

```{r id9932ddddddd2222, echo=FALSE, cache=TRUE,fig.width=4,fig.height=3.75}

gaze4_all_samples<-as.data.frame(fread('/Users/carlylasagna/University of Michigan Dropbox/Carly Lasagna/ddm_gaze_psychophys/ddm/output/hddm_m4_psychophys/gender/init_fit/allgroups_init_fit_samples.csv'))

colnames(gaze4_all_samples) <- gsub("\\[|\\]", ".", colnames(gaze4_all_samples))
gaze4_all_samples<-gaze4_all_samples[,-grep("sub_",colnames(gaze4_all_samples))]

gaze4_all_samples$mu_grp_delta_bias_pr.1.<-(gaze4_all_samples$mu_grp_delta_male_pr.1.+gaze4_all_samples$mu_grp_delta_female_pr.1.)/2
gaze4_all_samples$mu_grp_delta_bias_pr.2.<-(gaze4_all_samples$mu_grp_delta_male_pr.2.+gaze4_all_samples$mu_grp_delta_female_pr.2.)/2

gaze4_all_samples$mu_grp_delta_pr.1.<-(gaze4_all_samples$mu_grp_delta_male_pr.1.-gaze4_all_samples$mu_grp_delta_female_pr.1.)/2
gaze4_all_samples$mu_grp_delta_pr.2.<-(gaze4_all_samples$mu_grp_delta_male_pr.2.-gaze4_all_samples$mu_grp_delta_female_pr.2.)/2

gaze4_all_samples$mu_delta.1.<-pnorm(gaze4_all_samples$mu_grp_delta_pr.1.)*10-5
gaze4_all_samples$mu_delta.2.<-pnorm(gaze4_all_samples$mu_grp_delta_pr.2.)*10-5

gaze4_all_samples$mu_delta_bias.1.<-pnorm(gaze4_all_samples$mu_grp_delta_bias_pr.1.)*10-5
gaze4_all_samples$mu_delta_bias.2.<-pnorm(gaze4_all_samples$mu_grp_delta_bias_pr.2.)*10-5

params<-c('mu_alpha.','mu_beta.','mu_ndt.','mu_delta.','mu_delta_bias.','mu_grp_b1_pr.')
raw_params<-c('mu_grp_alpha_pr.','mu_grp_beta_pr.','mu_grp_ndt_pr.','mu_grp_delta_pr.','mu_grp_delta_bias_pr.','mu_grp_b1_pr.')
sig_params<-c('sig_grp_alpha_pr.','sig_grp_beta_pr.','sig_grp_ndt_pr.','sig_grp_delta_pr.','sig_grp_delta_pr.','sig_grp_b1_pr.')
param_names<-c('Threshold Separation','Start Point','NDT','Drift Sensitivity (Gender)','Drift Bias','Drift Sensitivity (Gaze)')

#loop through and plot/compare group differences
for(i in 1:length(params)){

  p<-plot_posterior_diffs(
    data=gaze4_all_samples,
    ci=.95, #credible interval
    par_prefix=params[i],
    rawpar_prefix=raw_params[i],
    sig_prefix=sig_params[i],
...
## Sensitivity - Group Differences Accounting for Right Censoring in Visual Integration Task

Due to the task design the Visual Integration task had right censoring of the data after 2500ms due to a response cutoff. We modeled the data with a DDM that also accounted for censoring to ensure the results did not change.

### Model 7 (Visual Integration) + Censoring

The Visual Integration task had right censoring past 2.5 ms (i.e., there was a 2.5s response window and this lead to a censoring of the RT distribution and choice data due to many responses taking longer than that). Therefore, we performed a sensitivity analysis using a DDM model that explicitly accounted for this right censoring and re-ran the group differences to ensure the results did not change.

```{r id9933212233222322999, echo=FALSE, cache=TRUE,fig.width=4,fig.height=3.75}

gaze7_all_samples<-as.data.frame(fread('/Users/carlylasagna/University of Michigan Dropbox/Carly Lasagna/ddm_gaze_psychophys/ddm/output/hddm_m7_psychophys/jovi/init_fit/allgroups_init_fit_samples.csv'))

colnames(gaze7_all_samples) <- gsub("\\[|\\]", ".", colnames(gaze7_all_samples))
gaze7_all_samples<-gaze7_all_samples[,-grep("sub_",colnames(gaze7_all_samples))]

# calculate and plot group differences
params<-c('mu_alpha.','mu_beta.','mu_ndt.','mu_grp_b1_pr.','mu_delta.')

raw_params<-c('mu_grp_alpha_pr.','mu_grp_beta_pr.','mu_grp_ndt_pr.',
              'mu_grp_b1_pr.', 'mu_grp_delta_pr.')

sig_params<-c('sig_grp_alpha_pr.','sig_grp_beta_pr.','sig_grp_ndt_pr.',
              'sig_grp_b1_pr.','sig_grp_delta_pr.')

param_names<-c('Threshold Separation','Start Point','NDT',
               'Drift Sensitivity',
               'Drift Bias')

colnames(gaze7_all_samples)<-paste0(colnames(gaze7_all_samples),".")

#loop through and plot/compare group differences
for(i in 1:length(params)){

  p<-plot_posterior_diffs(
    data=gaze7_all_samples,
    ci=.95, #credible interval
    par_prefix=params[i],
    rawpar_prefix=raw_params[i],
    sig_prefix=sig_params[i],
...
```{r id99329291112121331, fig.height=4, fig.width=4.5, message=FALSE, warning=FALSE, cache=TRUE, include=FALSE}

library(ggeffects)
library(ggplot2)

clean_data$group<-factor(clean_data$sz_dummy,levels=c(0,1),labels=c("HC","SZ"))

plot_data<-data.frame(age=clean_data$age,
                      female_dummy=clean_data$female_dummy,
                      Group=clean_data$group,
                      Drift_Sensitivity=clean_data$delta_b1,
                      GPTS_RefThinking=clean_data$gpts_socref)

out<-lm(GPTS_RefThinking~age+female_dummy+Group+Drift_Sensitivity+Group:Drift_Sensitivity,data=plot_data)

ggpredict(out, terms = c("Drift_Sensitivity", "Group")) |>
  plot(add.data = TRUE, dot.alpha = 0.8, dot.size = 2) +
  theme_bw(base_size = 14) +ggtitle("")+xlab("Drift Sensitivity")+ylab("GPTS Referential Thinking")+scale_color_manual(values=c('#57b6c9','#966acc'))+scale_fill_manual(values=c('#57b6c9','#966acc'))

plot_data<-data.frame(age=clean_data$age,
                      female_dummy=clean_data$female_dummy,
                      Group=clean_data$group,
                      Drift_Sensitivity=clean_data$delta_gend,
                      GPTS_RefThinking=clean_data$gpts_socref)
...
plot_data<-data.frame(age=clean_data$age,
                      female_dummy=clean_data$female_dummy,
                      Group=clean_data$group,
                      Drift_Sensitivity=clean_data$delta_b1_jovi,
                      GPTS_RefThinking=clean_data$gpts_socref)
...
plot_data<-data_frame(age=clean_data$age,
                      female_dummy=clean_data$female_dummy,
                      Group=clean_data$group,
                      Drift_Sensitivity=clean_data$delta_b1,
                      GPTS_Persecution=clean_data$gpts_persec)

out<-lm(GPTS_Persecution~age+female_dummy+Group+Drift_Sensitivity+Group:Drift_Sensitivity,data=plot_data)
summary(out)

ggpredict(out, terms = c("Drift_Sensitivity", "Group")) |>
  plot(add.data = TRUE, dot.alpha = 0.8, dot.size = 1.8) +
  theme_bw(base_size = 11) 

```

```{r id993292222911121331121, fig.height=4, fig.width=4.5, message=FALSE, warning=FALSE, cache=TRUE, include=FALSE}
...
ggpredict(out, terms = c("Drift_Sensitivity", "Group")) |>
  plot(add.data = TRUE, dot.alpha = 0.8, dot.size = 2) +
  theme_bw(base_size = 14) +ggtitle("")+xlab("Drift Sensitivity")+ylab("GPTS Persecution")+scale_color_manual(values=c('#57b6c9','#966acc'))+scale_fill_manual(values=c('#57b6c9','#966acc'))
...
## Sensitivity - Controlling for General Cognition {.tabset}
...
## Sensitivity - Controlling for General Perceptual Processing {.tabset}
...
## Sensitivity - Controlling for General Perceptual Processing (+ Censoring) {.tabset}

The Visual Integration task had right censoring past 2.5 ms (i.e., there was a 2.5s response window and this lead to a censoring of the RT distribution and choice data due to many responses taking longer than that). Therefore, we performed a sensitivity analysis using a DDM model that explicitly accounted for this right censoring and re-ran the regressions to ensure the results did not change.

### Referential Thinking

```{r id993299111221ss9221331, echo=FALSE, fig.height=4, fig.width=5, message=FALSE, warning=FALSE, cache=TRUE}
#
all_sub_data<-read.csv("/Users/carlylasagna/University of Michigan Dropbox/Carly Lasagna/ddm_gaze_psychophys/ddm/data/schizgaze2_all_sub_params_w_correlates.csv")

tmp_pred<-c('delta_b1','delta_gend','sz_dummy','female_dummy','age','delta_b1_jovi_c')
tmp_outcome<-c('gpts_socref')
...
#define models to test (list predictors in order they should be entered)
test_models<-list(c("age","female_dummy","sz_dummy","delta_b1_jovi_c"),
                  c("age","female_dummy","sz_dummy","delta_b1_jovi_c","delta_b1","sz_dummy:delta_b1"),
                  c("age","female_dummy","sz_dummy","delta_b1_jovi_c","delta_gend","sz_dummy:delta_gend"))

outcome<-"gpts_socref" #outcome variable
outfile<-paste0(dirname,"/analysis/full/saved_brms_models_1a2bb.rds")
...
result_glm_ddm$Pred<-gsub("_c",": Censor Model",result_glm_ddm$Pred)

result_glm_ddm %>%
  kbl(caption = "Bayesian Regression: Predicting GPTS Referential Thinking (Full Sample, Controlling for Age, Sex, General Perceptual Processing [Visual Integration + Censoring])",valign = "t") %>%
  kable_classic(full_width=F, html_font = "Cambria") %>%
  kable_styling(full_width=T)
...
```{r id9932991112122222239312121, echo=FALSE, fig.height=4, fig.width=5, message=FALSE, warning=FALSE, cache=TRUE}
#
all_sub_data<-read.csv("/Users/carlylasagna/University of Michigan Dropbox/Carly Lasagna/ddm_gaze_psychophys/ddm/data/schizgaze2_all_sub_params_w_correlates.csv")

tmp_pred<-c('delta_b1','delta_gend','delta_b1_jovi_c','sz_dummy','female_dummy','age')
tmp_outcome<-c('gpts_persec')
...
outcome<-"gpts_persec" #outcome
outfile<-paste0(dirname,"/analysis/full/saved_brms_models_2a2bb.rds")
...
result_glm_ddm %>%
  kbl(caption = "Bayesian Regression: Predicting GPTS Persecution (Full Sample, Controlling for Age, Sex, General Perceptual Processing [Visual Integration + Censoring])",valign = "t") %>%
  kable_classic(full_width=F, html_font = "Cambria") %>%
  kable_styling(full_width=T)
...
```{r id9932922222222222911129, echo=FALSE, fig.height=4, fig.width=5, message=FALSE, warning=FALSE, cache=TRUE,fig.show='asis',results='asis'}
#
all_sub_data<-read.csv("/Users/carlylasagna/University of Michigan Dropbox/Carly Lasagna/ddm_gaze_psychophys/ddm/data/schizgaze2_all_sub_params_w_correlates.csv")

show_modelchange<-TRUE
options(mc.cores = parallel::detectCores())
tmp_pred<-c('delta_b1','delta_gend','age','female_dummy','delta_b1_jovi_c')
tmp_outcome<-c('gpts_socref')
tmp_data<-subset(all_sub_data,all_sub_data$group=="SZ")
tmp_corr_data<-dplyr::select(tmp_data,c(tmp_pred,tmp_outcome))
clean_data<-na.omit(tmp_corr_data) #keep complete cases only
...
outcome<-"gpts_socref"
outcome_name<-"GPTS Referential Thinking (SZ Only)"
outcome_prefix<-"1.1.1.1"

for(i in 1:length(pars)){

  #define models to test (list predictors in order they should be entered)
  test_models<-list(c("age","female_dummy","delta_b1_jovi_c"),
                    c("age","female_dummy","delta_b1_jovi_c",pars[i]))
...
```{r id993299111222222222222218, echo=FALSE, fig.height=4, fig.width=5, message=FALSE, warning=FALSE, cache=TRUE,fig.show='asis',results='asis'}
#
all_sub_data<-read.csv("/Users/carlylasagna/University of Michigan Dropbox/Carly Lasagna/ddm_gaze_psychophys/ddm/data/schizgaze2_all_sub_params_w_correlates.csv")
...
__Re-run those follow-up within group regressions to see if effects remain after controlling for general perceptual processing ability (i.e., drift sensitivity from Visual Integration task)__
...
```{r id9932993423424234131121, echo=FALSE, fig.height=4, fig.show='asis', fig.width=5, message=FALSE, warning=FALSE, cache=TRUE, results='asis'}
#
all_sub_data<-read.csv("/Users/carlylasagna/University of Michigan Dropbox/Carly Lasagna/ddm_gaze_psychophys/ddm/data/schizgaze2_all_sub_params_w_correlates.csv")
...
```{r id9983299111212233333322891, echo=FALSE, fig.height=4, fig.show='asis', fig.width=5, message=FALSE, warning=FALSE, cache=TRUE, results='asis'}
#
all_sub_data<-read.csv("/Users/carlylasagna/University of Michigan Dropbox/Carly Lasagna/ddm_gaze_psychophys/ddm/data/schizgaze2_all_sub_params_w_correlates.csv")
...
# Exploratory Correlations with Other Clinical Measures in SZ

```{r id99832992222111212222891, echo=FALSE, fig.height=8.5, fig.show='asis', fig.width=9, message=FALSE, warning=FALSE, cache=TRUE, results='asis'}
...
lm_matrix<-function(outcomes,covariates,predictors,data){
  ...
  # create empty matrix to hold outputs
  corr_mat<-as.data.frame(matrix(data="",ncol=n_outcome,nrow=n_pred))
  rownames(corr_mat)<-predictors
  colnames(corr_mat)<-outcomes
  
  #create correlation matrices to hold summary stats
  corr_mat_range<-corr_mat_r<-corr_mat_p<-corr_mat
  
  iteration<-0
  
  for (i in 1:n_pred){ #var1 / predictors
    for (j in 1:n_outcome){ #var2 / outcomes
      
      var1<-predictors[i] #name of variable1
      var2<-outcomes[j] #name of variable2
      ...
      if(var1!=var2){ #if are not equal, run regressions
          iteration<-iteration+1
          
          form <- as.formula(paste("scale(",var2, ")~",paste(covariates,collapse="+"),"+scale(", var1,")"))
          mod <- lm(form, data = data)
          mod_summary <- summary(mod)
          r<-round(mod_summary$coefficients[paste0("scale(",var1,")"), "Estimate"],2)
          p<-mod_summary$coefficients[paste0("scale(",var1,")"), "Pr(>|t|)"]
  ...
  var_list<-data.frame(Variable=rownames(corr_mat_r))
  
  library(tidyr)
  library(dplyr)

  tmp <- as.data.frame(corr_mat_r)
  tmp$Variable <- rownames(corr_mat_r)
  ...
  colnames(df_long)<-c("Predictor","Outcome","r")
  df_long$r<-as.numeric(df_long$r)
  
  #do the same for p
  tmp <- as.data.frame(corr_mat_p)
  tmp$Variable <- rownames(corr_mat_p)
  ...
  colnames(df_long_p)<-c("Predictor","Outcome","p")
  df_long_p$p<-as.numeric(df_long_p$p)
  
  #do the same for p range
  tmp <- as.data.frame(corr_mat_range)
  tmp$Variable <- rownames(corr_mat_range)
  ...
  colnames(df_long_range)<-c("Predictor","Outcome","range")
  
  # Create the plot 
  p <- ggplot(df_long, aes(x = factor(Predictor, levels = predictors), y = factor(Outcome, levels = outcomes))) +
    geom_tile(aes(fill = r), color = "black") +
    geom_text(aes(label = round(r, 2)), color = "black", vjust = 0.3) +  # Add r values
    geom_text(data = df_long_range, aes(label = range), color = "black", vjust = 1.5,size=6) +  # Add credible values
    scale_fill_gradient2(low = "steelblue", high = "firebrick", midpoint = 0, na.value = "transparent", guide = "legend") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    labs(title = "Regression Summary",
         x = "", y = "",
         fill = "Standardized Betas")
  
  rownames(all_long_summary)<-NULL
  
  combined<-list(long_summary=all_long_summary,
                 corr_mat_r=corr_mat_r,
                 corr_mat_range=corr_mat_range,
                 corr_mat_p=corr_mat_p,
                 plot=p)

  return(combined)
}

corr_data<-all_sub_data[c("subj","group","bprs","sans","saps_delus_persec","saps_delus_idea_ref","gf_role","gf_social","bdi","stai_state","delta_b1","delta_b1_jovi","delta_gend","age","gender")]

corr_data$gender<-ifelse(corr_data$gender=="Male",0,1)

corr_sz<-subset(corr_data,corr_data$group=="SZ")

corr_sz$group<-corr_sz$subj<-NULL
# 
# out<-lm_matrix(outcomes=c("bprs","sans","saps_delus_persec","saps_delus_idea_ref","gf_role","gf_social","stai_state"),
#                covariates=c("age","gender"),
#                predictors=c("delta_b1","delta_gend","delta_b1_jovi"),
#                data=corr_sz)
# 
# out[["plot"]]

corr<-corr_sz[,c("delta_b1","delta_gend","delta_b1_jovi",
                 "bprs","sans","saps_delus_persec","saps_delus_idea_ref",
                 "gf_role","gf_social","stai_state","bdi")]

colnames(corr)<-c("Drift Sensitivity (Gaze)","Drift Sensitivity (Gender)","Drift Sensitivity (Visual Integ.)",
                 "BPRS","SANS","SAPS Persecution","SAPS Ideas of Reference",
                 "GF Role","GF Social","STAI (Anxiety)","BDI (Depression)")

out<-bayes_corr_matrix(data=corr,credMass=.95)
out[["plot"]]
...
Andreasen, N. (1984a). Scale for the assessment of positive symptoms (SAPS). The University of Iowa. https://iro.uiowa.edu/esploro/outputs/other/Scale-for-the-assessment-of-positive/9984097080502771
...
Couture, S., Grant, P., Beck, A., & Morrison, A. (2007). Success and resource appraisals questionnaire (SARA-Q). Unpublished.
...
Patterson, T. L., Goldman, S., McKibbin, C. L., Hughs, T., & Jeste, D. V. (2001). UCSD Performance-Based Skills Assessment: Development of a new measure of everyday functioning for severely mentally ill adults. Schizophrenia Bulletin, 27(2), 235–245. https://doi.org/10.1093/oxfordjournals.schbul.a006870

Patterson, T. L., Moscona, S., McKibbin, C. L., Davidson, K., & Jeste, D. V. (2001). Social skills performance assessment among older patients with schizophrenia. Schizophrenia Research, 48(2), 351–360. https://doi.org/10.1016/S0920-9964(00)00109-2
...
Snelbaker, A. J., Wilkinson, G. S., Robertson, G. J., & Glutting, J. J. (2001). Wide Range Achievement Test 3 (wrat3). In Understanding Psychological Assessment (pp. 259–274). Springer US. https://doi.org/10.1007/978-1-4615-1185-4_13

Spielberger, C. D., Gorsuch, R. L., Lushene, R., Vagg, P. R., & Jacobs, G. A. (2003). State–Trait Anxiety Inventory Self Evaluation Questionnaire, Form Y (STAI) (P. Lindley & D. Bartram, Eds.). British Psychological Society. https://doi.org/10.53841/bpstest.2003.stai
...
Tso, I. F., Grove, T. B., & Taylor, S. F. (2012). Self-assessment of psychological stress in schizophrenia: Preliminary evidence of reliability and validity. Psychiatry Research, 195(1–2), 39–44. https://doi.org/10.1016/j.psychres.2011.07.009
